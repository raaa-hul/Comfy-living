...
function OfficeProducts() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await axios.get("http://localhost:8000/api/v1/furniture/?category=office"); // Fetch office products
  ...
  return (
    <div className="products-bg mt-5 mb-5">
      ...
      <p className="text-xl text-white text-opacity-80 max-w-3xl mx-auto text-center mb-6">
        Explore our stylish and functional office furniture collection!
      </p>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-6 px-4">
        {products.map((product) => (
          <div
            key={product.id}
            className="bg-white shadow-md rounded-lg overflow-hidden transition-transform duration-300 transform hover:scale-105 hover:shadow-lg"
          >
            <div className="">
